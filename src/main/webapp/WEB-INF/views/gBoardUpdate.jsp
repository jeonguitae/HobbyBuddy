...
<jsp:include page="gnb.jsp"/>
	<form action="gupdate.do" method="post" enctype="multipart/form-data">
		<table>
			<tr hidden>
				<th>글 번호</th>
				<td><input type="text" name="gidx" value="${board.gidx}" readonly/></td>
			</tr>
			<tr>
				<th>작성자</th>
			...
			<tr>
			 <th>취미</th>
	         <td>
	         	<input type="text" name="big_hb" value="${board.big_hb}" readonly/>
	         	<input type="text" name="small_hb" value="${board.small_hb}" readonly/>
	         </td>
      		</tr>
				<tr>
					<th>나이</th>
				...
				<tr>
					...
					<td>
						<select name="city">
							<option class="mycity" value="${board.city}">${board.city}</option>
						...
						<select name="area">
							<option class="mycity" value="${board.area}">${board.area}</option>
				...
				<tr>
					<th>사진</th>
					<td>
						<c:if test="${board.new_photo_name eq null}">
							<input type="file" name="photo"/>
						</c:if>
						<c:if test="${board.new_photo_name ne null}">
							<img width="500" src="/photo/${board.new_photo_name}"/>
							<input type="hidden" name="id" value="${sessionScope.loginId}"> 
							<!-- 사진 삭제 시 사진의 idx를 갖고 와야 하니까 : hidden-->
					  		<input type="hidden" name="photo_idx" value="${board.photo_idx}">
					  		${board.board_class}
					  	<a href="gPhotodel.do?gidx=${board.gidx}&id=${board.id}">사진 삭제</a>	
						</c:if>			
					</td>
				</tr>
				...
				<tr>
					<th>최대 인원</th>
					<td>
						<input type="number" name="maxmem" value="${board.maxmem}"/>
					</td>
				</tr>
				...
				<tr>
					<th>매너온도</th>
					<td>
						<input type="number" name="mannertp" value="${board.mannertp}"/>
					</td>
				</tr>
